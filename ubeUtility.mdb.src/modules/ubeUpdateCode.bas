...
Private Function GetBEDBPath(Optional ByVal knownTable As String = vbNullString) As String
    ...
    If knownTable = vbNullString Then
        ...
        For Each theTable In thisDatabase.TableDefs          'loop thru all tables
            If (theTable.Attributes And dbSystemObject) = 0 Then 'skip system tables
                If Nz(theTable.Connect) <> vbNullString Then 'if table is linked then
                    pathName = theTable.Connect          'fetch connection string
                    Exit For                         'short circuit
                End If
            End If
        Next theTable
    Else
        pathName = GetConnectFromTable(knownTable)
    End If
    
    ' remove everything before ';DATABASE='
    GetBEDBPath = Mid$(pathName, InStrRev(pathName, "=") + 1)
...
Private Function SchemaUpdate(ByVal vVersion As Long, ByVal vDeveloper As Boolean) As Boolean
    ...
    Do Until updateList.EOF                             'step thru list
        ...
        Case "Copy Table"
            TableName = updateList.Fields.Item("TableName").Value            'fetch table name
            If TableExists("ube" & TableName) = False Then                     'if ube'Table' not exists then
                DoCmd.CopyObject vPathname, , acTable, TableName               'copy table to back-end file
                DoCmd.Rename "ube" & TableName, acTable, TableName            'prefix table name with 'ube'
                'DoCmd.TransferDatabase acLink, "Microsoft Access", vPathname, acTable, tableName, tableName 'set link to new table
            Else                                 'if ube'Table' exists then
                DoCmd.CopyObject vPathname, TableName, acTable, "ube" & TableName 'copy table to back-end and rename
            End If
            
            If TableExists(TableName) = True Then DoCmd.DeleteObject acTable, TableName 'if link exists then delete Link
            DoCmd.TransferDatabase acLink, "Microsoft Access", vPathname, acTable, TableName, TableName 'and re-link to new table in BE
            backendDB.TableDefs.Refresh                 'refresh table collection
        ...
        Case "New Field"
            If updateList.Fields.Item("FieldType").Value = "ATTACHMENT" Then 'if field type = 'ATTACHMENT'
                Set theTable = backendDB.TableDefs.Item(updateList.Fields.Item("TableName")) 'set ref to specified table
                theTable.Fields.Append theTable.CreateField(updateList.Fields.Item("FieldName"), 101) 'add Attachment type field (101 = dbAttachment)
                
                Dim dbLocal As DAO.Database
                Set dbLocal = CurrentDb()        'refresh links to BE (due to bug in A2007)
                For Each theTable In dbLocal.TableDefs 'loop through all tables
                    If theTable.Name = updateList.Fields.Item("TableName").Value Then 'skip if not current table
                        theTable.Connect = ";DATABASE=" & vPathname 'set pathname + filename of back-end
                        theTable.RefreshLink          'and make link to back end
                    End If
                Next theTable
            ...
            Else
                If updateList.Fields.Item("FieldType").Value = "HYPERLINK" Then 'if field type = 'HYPERLINK'
                    Set theTable = backendDB.TableDefs.Item(updateList.Fields.Item("TableName")) 'set ref to curretn table
                    Set theField = theTable.CreateField(updateList.Fields.Item("FieldName"), dbMemo) 'add Memo field first
                    theField.Attributes = dbHyperlinkField 'set attribute to Hyperlink
                    theTable.Fields.Append theField        'and append field to table
...
Private Function TableExists(ByVal TableName As String) As Boolean
    ...
    TableExists = (thisDatabase.TableDefs.Item(TableName).Name = TableName)
    Exit Function
...
Public Function RelationExists(ByVal relationName As String) As Boolean
...
'''Entry  (tableName) = name of table to change
...
Private Sub SetProperties(ByRef TableName As String, ByRef FieldName As String, ByVal propertyType As String, ByRef requiredParameters As String, ByRef Description As String)
    ...
    Case "Text Field Size ="
        If backendDB.TableDefs.Item(TableName).Fields.Item(FieldName).Type = dbText Then 'if Field type = TEXT then
            backendDB.Execute "ALTER TABLE [" & TableName & "] ALTER COLUMN [" & FieldName & "] TEXT (" & requiredParameters & ")" 'change field size
        End If
    Case "Set Compression"
        returnValue = adhSetProp(backendDB.TableDefs.Item(TableName).Fields.Item(FieldName), "UniCodeCompression", True) 'set UniCode Compression
    Case "Required ="
        If requiredParameters = "Yes" Or requiredParameters = "True" Then setStatus = True 'convert Yes/True to Boolean
        returnValue = adhSetProp(backendDB.TableDefs.Item(TableName).Fields.Item(FieldName), "Required", setStatus) 'set set Required Yes/No
    Case "Allow Zero Len ="
        If requiredParameters = "Yes" Or requiredParameters = "True" Then setStatus = True 'convert Yes/True to Boolean
        returnValue = adhSetProp(backendDB.TableDefs.Item(TableName).Fields.Item(FieldName), "AllowZeroLength", setStatus) 'set Allow Zero Length
    Case "Default Value ="
        returnValue = adhSetProp(backendDB.TableDefs.Item(TableName).Fields.Item(FieldName), "DefaultValue", requiredParameters) 'set Default Value
    Case "Input Mask ="
        returnValue = adhSetProp(thisDatabase.TableDefs.Item(TableName).Fields.Item(FieldName), "InputMask", requiredParameters) 'set Input Mask Value
    Case "Format ="
        returnValue = adhSetProp(thisDatabase.TableDefs.Item(TableName).Fields.Item(FieldName), "Format", requiredParameters) 'set Format type
    Case "Decimal Places ="
        returnValue = adhSetProp(thisDatabase.TableDefs.Item(TableName).Fields.Item(FieldName), "DecimalPlaces", requiredParameters) 'set Decimal Places
    Case "Validation Rule ="
        returnValue = adhSetProp(backendDB.TableDefs.Item(TableName).Fields.Item(FieldName), "ValidationRule", requiredParameters) 'set Validation Rule
        returnValue = adhSetProp(backendDB.TableDefs.Item(TableName).Fields.Item(FieldName), "ValidationText", Description) 'set Validation Text
    Case "New Field Name ="
        returnValue = adhSetProp(backendDB.TableDefs.Item(TableName).Fields.Item(FieldName), "Name", requiredParameters) 'set/change Field Name
    Case "Description ="
        returnValue = adhSetProp(backendDB.TableDefs.Item(TableName).Fields.Item(FieldName), "Description", Description) 'set Description field
    Case "Ordinal Position ="
        returnValue = adhSetProp(backendDB.TableDefs.Item(TableName).Fields.Item(FieldName), "OrdinalPosition", requiredParameters) 'set Field Ordinal posn
    Case "Set Primary Key ="
        If requiredParameters <> vbNullString Then FieldName = FieldName & "," & requiredParameters 'add in extra fields (if any)
        returnValue = adhCreatePrimaryKey(TableName, "PrimaryKey", FieldName) 'set Primary Key/s
    Case "Indexed (No)"
        returnValue = FindIndex(TableName, FieldName)   'find Index name
        If returnValue <> vbNullString Then backendDB.Execute "DROP INDEX [" & returnValue & "] ON [" & TableName & "]" 'remove Index (if any)
    Case "Indexed (Dup OK)"
        returnValue = FindIndex(TableName, FieldName)   'find Index name
        If returnValue <> vbNullString Then backendDB.Execute "DROP INDEX [" & returnValue & "] ON [" & TableName & "]" 'remove Index (if any)
        backendDB.Execute "CREATE INDEX [" & FieldName & "] ON [" & TableName & "] ([" & FieldName & "])" 'set Index (use Field name as Index name)
    Case "Indexed (No Dup)"
        returnValue = FindIndex(TableName, FieldName)   'find Index name
        If returnValue <> vbNullString Then backendDB.Execute "DROP INDEX [" & returnValue & "] ON [" & TableName & "]" 'remove Index (if any)
        backendDB.Execute "CREATE UNIQUE INDEX [" & FieldName & "] ON [" & TableName & "] ([" & FieldName & "])" 'set unique Index (use Field name as Index name)
    Case "Fill With ="
        FillField TableName, FieldName, requiredParameters 'copy data to field
    Case "Rich Text ="
        If requiredParameters = "Yes" Or requiredParameters = "True" Then isRTF = 1 Else isRTF = 0 'convert Yes/True to 1/0
        returnValue = adhSetProp(backendDB.TableDefs.Item(TableName).Fields.Item(FieldName), "TextFormat", isRTF) 'set/clear Rich Text format
        returnValue = adhSetProp(backendDB.TableDefs.Item(TableName).Fields.Item(FieldName), "Description", Description) 'set Description field
    Case "Caption Name ="
        returnValue = adhSetProp(backendDB.TableDefs.Item(TableName).Fields.Item(FieldName), "Caption", requiredParameters) 'set Caption property
        returnValue = adhSetProp(backendDB.TableDefs.Item(TableName).Fields.Item(FieldName), "Description", Description) 'set Description field
    Case "Smart Tags ="
        returnValue = adhSetProp(backendDB.TableDefs.Item(TableName).Fields.Item(FieldName), "SmartTags", requiredParameters) 'set Smart Tags property
        returnValue = adhSetProp(backendDB.TableDefs.Item(TableName).Fields.Item(FieldName), "Description", Description) 'set Description field
    End Select
...
'''Entry  (tableName) = name of table to fill
...
'''       Any errors handled by main UpdateBackEndFile routine
Private Sub FillField(ByVal TableName As String, ByVal vFieldName As String, ByRef vParameter As String)

    Dim vFieldType As Long
    Dim vData As String
    Const QUOTE As String = """"                           'Used in place of Double Quotes

    vFieldType = backendDB.TableDefs.Item(TableName).Fields.Item(vFieldName).Type 'fetch field type
    ...
    Case dbText, dbMemo                          'if Text or Memo then
        ...
        backendDB.Execute "UPDATE [" & TableName & "] SET [" & vFieldName & "] = " & vData & " WHERE [" & vFieldName & "] IS NULL" 'fill field for all blank records
    Case dbDate                                  'if date or time
        vData = "#" & Format$(vParameter, "mm\/dd\/yyyy") & "#" 'reformat date for US mode
        backendDB.Execute "UPDATE [" & TableName & "] SET [" & vFieldName & "] = " & vData & " WHERE [" & vFieldName & "] IS NULL" 'fill field for all blank records
    Case dbBoolean
        backendDB.Execute "UPDATE [" & TableName & "] SET [" & vFieldName & "] = " & vParameter 'set field to True or False
    Case Else
...
'''       Any errors handled by main UpdateBackEndFile routine
Private Sub NewFieldDefaults(ByVal TableName As String, ByVal vFieldName As String, ByVal vFieldType As String, ByRef vDescription As String)

    Dim returnValue As Variant
    Dim theField As DAO.Field

    returnValue = adhSetProp(backendDB.TableDefs.Item(TableName).Fields.Item(vFieldName), "Description", vDescription) 'set Description field (if any)
    Select Case vFieldType
    Case "BYTE", "SHORT", "LONG", "SINGLE", "DOUBLE", "CURRENCY" 'select Number fields only
        returnValue = adhSetProp(backendDB.TableDefs.Item(TableName).Fields.Item(vFieldName), "DefaultValue", 0) 'set Number fields Default Value to 0
    Case "TEXT", "MEMO", "HYPERLINK"             'select TEXT & MEMO & HYPERLINK fields
        returnValue = adhSetProp(backendDB.TableDefs.Item(TableName).Fields.Item(vFieldName), "UniCodeCompression", True) 'always set UniCode Compression on Text/Memo
        returnValue = adhSetProp(backendDB.TableDefs.Item(TableName).Fields.Item(vFieldName), "AllowZeroLength", True) 'always set AllowZeroLength on Text/Memo
    Case "YESNO"                                 'select Yes/No fields
        Set theField = backendDB.TableDefs.Item(TableName).Fields.Item(vFieldName) 'change YesNo field format
        SetPropertyDAO theField, "DisplayControl", dbInteger, CInt(acCheckBox) 'to Check Box type
    End Select
...
'''Set some properties for new fields regardless
'''Entry  (tableName) = name of table with indexed field
'''       (vFieldName) = name of indexed field
'''       (db) = Database object referenced to back-end file
'''Exit   Index name for selected table/field returned or "" if none
'''       Any errors handled by main UpdateBackEndFile routine
Private Function FindIndex(ByVal TableName As String, ByVal vFieldName As String) As String

    Dim idx As DAO.Index
    Dim theTable As DAO.TableDef
    
    With backendDB.TableDefs
        Set theTable = .Item(TableName)           'define required table
    End With
    
    For Each idx In theTable.Indexes                  'search Indexes
...
Private Function CreateRelationship(ByVal vPKTableName As String, ByVal vPKFieldName As String, ByVal vRelationshipType As String, ByVal vFKTableName As String, ByVal vFKFieldName As String) As Boolean
    ...
    If RelationExists(vRelationshipName) Then
        ...
        For Each savedField In existingFields.Keys
            Set theField = rel.CreateField(existingFields(savedField))   'Set the relation's field collection.
            theField.ForeignName = existingFields.Item(savedField)     'set Foreign table field
            rel.Fields.Append theField                    'append foreign field
        Next savedField
    ...
    Set theField = rel.CreateField(vPKFieldName)      'Set the relation's field collection.
    theField.ForeignName = vFKFieldName               'set Foreign table field
    rel.Fields.Append theField                        'append foreign field
...
Private Function adhSetProp(ByVal obj As Object, ByRef propertyName As String, ByRef propertyValue As Variant) As Variant
    ...
    oldValue = obj.Properties(propertyName)
...
adhSetProp_Err:
    ...
    Case adhcErrNotInCollection
        returnValue = AddProp(obj, propertyName, propertyValue)
        If IsError(returnValue) Then
            adhSetProp = returnValue
        Else
            Resume Next
        End If
    Case Else
...
Private Function AddProp(ByVal obj As Object, ByRef propertyName As String, ByRef propertyValue As Variant) As Variant
    ...
    returnValue = adhGetDAOType(propertyValue)
    If IsError(returnValue) Then
    ...
    Else
        Set newProperty = obj.CreateProperty(propertyName, returnValue, propertyValue)
        obj.Properties.Append newProperty
        AddProp = True
    End If
...
''' in the caller.
Private Function adhGetDAOType(ByRef propertyValue As Variant) As Variant
...
Private Function adhCreatePrimaryKey(ByVal TableName As String, ByVal keyName As String, ByVal keyFields As String) As Boolean
    ...
    Dim theTable As DAO.TableDef
    Set theTable = backendDB.TableDefs.Item(TableName)

    ' Find out if the table currently has a primary key.
    ' If so, delete it now.
    Dim primaryKey As Variant
    primaryKey = FindPrimaryKey(theTable)
    If Not IsNull(primaryKey) Then
        With theTable.Indexes
            .Delete primaryKey
        End With
    End If
    ...
    Set newIndex = theTable.CreateIndex(keyName)
    ...
    theFields = Split(keyFields, ",")
    Dim I As Long
    For I = 0 To UBound(theFields)
        If Not AddField(newIndex, theFields(I)) Then adhCreatePrimaryKey = False
    Next I
...
Private Function FindPrimaryKey(ByVal theTable As DAO.TableDef) As Variant

    ' From Access 2000 Developer's Handbook by Litwin, Getz, and Gilbert (Sybex) Copyright 1999.  All rights reserved.
    Dim idx As DAO.Index

    For Each idx In theTable.Indexes
        If idx.Primary Then
...
Private Function AddField(ByVal theIndex As DAO.Index, ByVal FieldName As String) As Boolean
    ...
    If Len(FieldName & vbNullString) > 0 Then
        Set theField = theIndex.CreateField(FieldName)
        theIndex.Fields.Append theField
    End If
...
Private Function SetPropertyDAO(ByVal obj As Object, ByRef propertyName As String, ByRef propertyType As Long, ByRef propertyValue As Variant, Optional ByRef errorMsg As String) As Boolean

    On Error GoTo ErrHandler

    If HasProperty(obj, propertyName) Then
        obj.Properties(propertyName) = propertyValue
    Else
        obj.Properties.Append obj.CreateProperty(propertyName, propertyType, propertyValue)
    End If
...
ErrHandler:
    errorMsg = errorMsg & obj.Name & "." & propertyName & _
                " not set to " & propertyValue & ". Error " & Err.Number & " - " & _
...
Private Function HasProperty(ByVal obj As Object, ByRef propertyName As String) As Boolean

    On Error Resume Next
    
    Debug.Print obj.Properties(propertyName)
...
Private Function AddReferenceTable(ByRef TableName As String) As Boolean
    ...
    With OpenDatabase(bePathname)
        .Execute "CREATE TABLE [" & TableName & "] (Setting CHAR, DataValue CHAR)" 'create table Setting and Value
        .TableDefs.Refresh                         'refresh table collection
        .Close
    End With
    
    If TableExists(TableName) = False Then       'if vRefTable not exists then
        DoCmd.TransferDatabase acLink, "Microsoft Access", bePathname, acTable, TableName, TableName 'and re-link to new table in BE
    End If
